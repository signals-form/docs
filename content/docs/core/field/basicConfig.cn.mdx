...
- `hidden` 可以是一个配置对象，可以用来配置，字段隐藏后，字段值的删除与恢复，以及其子组件是否保活，在表单提交时，隐藏的字段是否也需要提交。
- `when` 是一个 `Decision` 对象，表示字段是否隐藏。`hidden` 也可以是一个函数。函数的参数是 `Field` 的`value` 以及表单模型 `model` ，返回值是一个布尔值。`hidden` 的默认值是 `false`。
- `activityOnHidden` 为 `true`时，表示当字段隐藏时，搭配 `Activity` 组件，对其字段组件进行保活，使得组件是不会被卸载，只会隐藏，默认为 `false`。
- `removeValueOnHidden` 为 `true`时，表示当字段隐藏时，字段的 `value` 会被删除，也就是置为 `undefined`，默认为 `true`。
- `recoverValueOnShown` 为`true`时，表示字段由隐藏转为显示时，字段的 `value` 会回填为隐藏时的值，默认为 `false`。
- `submitOnHidden` 为`true`时，表示字段隐藏时，表单提交时，也会被提交。
...
- 事件处理函数的 `this` 指向当前字段的 `handler` 对象，可以通过 `this.setState` 或 `this.setAsyncState` 来更新字段的状态。还有其他的状态和方法在后面的章节中会介绍。
